...
export class StorageAPI {
    ...
    async uploadFromText(path: string, content: string, options: {
        compress: boolean
    }) {
        return await this.#api.uploadFromText(path, content, options)
    }

    async uploadFromBytes(path: string, content: Buffer, options: {
        compress: boolean
    }) {
        return await this.#api.uploadFromBytes(path, content, options)
    }

    async uploadFromFilename(name: string, filename: string, options: {
        compress: boolean,
    }) {
        try {
    ...
    async delete(objectName: string, options: {
    ...
    async downloadAsStream(filename: string, options: {
        decompress: boolean
    }) {
        return await this.#api.downloadAsStream(filename, options)
    }

    async downloadAsText(filename: string, options: {
        decompress: boolean
    }) {
        return await this.#api.downloadAsText(filename, options)
    }

    async downloadAsBytes(filename: string, options: {
        decompress: boolean
    }) {
